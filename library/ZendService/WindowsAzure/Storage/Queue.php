...
class Queue extends Storage
{
     ...
     * @param bool                            $isAccountKeyBase64encoded
     */
    public function __construct($host = Storage::URL_DEV_QUEUE,
                                ...
                                $isAccountKeyBase64encoded = false
    )
    {
        parent::__construct($host, $accountName, $accountKey, $usePathStyleUri, $retryPolicy, $isAccountKeyBase64encoded);
     ...
     * @return mixed
     */
    public function popMessage($queueName)
    {
        $sasGenerator = new SharedAccessSignatureGenerator();
        $request = $sasGenerator->generateRequestObject(
            Request::METHOD_DELETE,
            $this->getBaseUrl()."/".trim($queueName, "/")."/messages/head",
            $this->_accountName,
            $this->_accountKey,
            time()+300
        );

        return $this->_retryPolicy->execute([$this->_httpClientChannel, 'send'], [$request]);
    }
}
