...
abstract class AbstractCredentials
{
     ...
     * @param boolean $isAccountKeyBase64encoded
     */
    public function __construct(
        $accountName = AbstractCredentials::DEVSTORE_ACCOUNT,
        $accountKey = AbstractCredentials::DEVSTORE_KEY,
        $usePathStyleUri = false,
        $isAccountKeyBase64encoded = true
    )
    {
        $this->_accountName     = $accountName;
        $this->_accountKey      = $isAccountKeyBase64encoded ? base64_decode($accountKey) : $accountKey;
        $this->_usePathStyleUri = $usePathStyleUri;
    }

    /**
     *nSet account name for Windows Azure
     *
     * @param  string $value
